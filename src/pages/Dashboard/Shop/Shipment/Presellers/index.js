                ...
                title: getLocales('Статус'), dataIndex: '', key: 'operations', itemClassName: 'text-center', headerClassName: 'text-center', render: (e, item) =>
                    <div className="sparkline8">
                        <button
                            ...
                            className={"btn  font-m auth-btn " + (item.status == 1 ? " btn-primary" : (item.status == 2 ? " btn-danger" : " btn-secondary"))}> {item.status == 1 ? getLocales('Доступно') : getLocales('Отключено')}
                        </button>
                    </div>
            },
            {
        ...
        return (
            <div className="row">
                <div className="col-lg-4">
                    <div class={"block animate__animated animate__fadeIn " + (this.state.loading ? "blur" : "")}>
                        <div class="block-body">
                            <h4 className="font-m">{getLocales('Добавление возможности предзаказа')}</h4>
                            ...
                            <button onClick={this.createSeller} disabled={this.state.loading} class="btn btn-primary font-m auth-btn margin-15">{this.state.loading ? <>{getLocales('Загрузка...')}</> : <>{getLocales('Добавить возможность предзаказа')}</>}</button>
                        </div>
                    </div>
                </div>
                <div className="col-lg-8">
                    <div class={"block animate__animated animate__fadeIn " + (this.state.loading ? "blur" : "")}>
                        <div class="block-body">
                            <h4 className="font-m">{getLocales('Доступные для предзаказа направления')}</h4>
                            ...
                            <div class="margin-15">
                                {
                                    this.state.items.length <= 0
                                        ?
                                        <div className="text-center font-m">{getLocales('Направления для предзаказа отсутствуют')}</div>
                                        :
                ...
                <ModalConfirm action={getLocales('Вы действительно хотите удалить данное направление предзаказа товара?')} consequences="" modal={this.state.confirmModal} toggle={this.confirmToggle} loading={this.state.loading} sendData={this.sellerDelete} />
            </div>
        )
    }
}

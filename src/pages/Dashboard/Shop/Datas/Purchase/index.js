                ...
                <div class="block-body">
                    <div className="row">
                        <div className="col-lg-12">
                            <h3 className="font-m">{getLocales('Покупка')} #{this.props.match.params.purchaseId} {(this.state.data.purchase.notfound == 0 && this.state.data.purchase.courier !== null )? <span className='right pointer' onClick={this.toggle}>{getLocales("Отметить как ненаход")}</span> : '' }</h3>
                        </div>
                        <div className='col-lg-4'>
                            <div class="form-group">
                                <label class="form-control-label font-m">{getLocales('Локация')}</label>
                        ...
                        <div className='col-lg-4'>
                            <div class="form-group">
                                <label class="form-control-label font-m">{getLocales('Покупатель')}</label>
                                <div class="input-group mb-3">
                                    <input name="nameUser" value={this.state.data.purchase.nameUser} class="form-control" disabled />
                                    {
                                        this.state.data.purchase.user !== 0
                                            ?
                                            <NavLink to={"/dashboard/shops/" + this.props.match.params.shopId + "/datas/users/" + this.state.data.purchase.user}><button class="btn btn-secondary font-m" type="button" id="button-addon2">{getLocales('Перейти в профиль')}</button></NavLink>
                                            :
                                            ''
                                    }
                                </div>
                        ...
                        <div className='col-lg-12'>
                            {
                                this.state.data.purchase.seller !== null
                                    ...
                                    <div className='avatar-block notice-chat font-m text-center'>
                                        ...
                                        <p>{getLocales('Адрес не был выдан, по причине того, что в наличии не было товара по данному направлению, после пополнения магазина адресами, заказам, с невыданным адресом по данному направлению будет автоматически распределяться добавленный товар, в порядке очереди.')}</p>
                                    </div>
                            }
                        </div>
                        <div className='col-lg-4'>
                            <div class="form-group">
                                <label class="form-control-label font-m">{getLocales('Сумма')}</label>
                        ...
                        <div className='col-lg-4'>
                            <div class="form-group">
                                <label class="form-control-label font-m">{getLocales('Дата завершения покупки')}</label>
                                <div class="input-group">
                        ...
                        </div>
                        {
                            this.state.data.purchase.courier == null
                                ...
                                <>
                                    ...
                                    <div className='col-lg-6'>
                                    <div class="form-group">
                                            <label class="form-control-label font-m">{getLocales('Заказ отмечен как ненаход')}</label>
                                            <input disabled value={this.state.data.purchase.notfound == 0 ? getLocales('Нет') : getLocales('Да')} class="form-control" />
                                        </div>
                                    </div>
                                </>
                        }
                    </div>
                    <button className='btn btn-secondary font-m' onClick={() => { this.props.history.goBack() }}><>{getLocales('Назад')}</></button>
                </div>

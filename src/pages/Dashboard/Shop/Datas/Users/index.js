...
class DataUsers extends Component {
    ...
    prepareTableData() {
        ...
        this.state.sorted.map((item) => {
            let itemModified = {
                ...
                platform: item.chatid == 0 ? getLocales('Сайт') : "Telegram"
            }
    ...
    render() {
        const tableColumns = [
            ...
            {
                title: getLocales('Имя'), dataIndex: 'name', key: 'operations', render: (e, item) => <NavLink to={"/dashboard/shops/" + this.props.match.params.shopId + "/datas/users/" + item.id}>{item.name}</NavLink>
            },
            {
            ...
            {
                title: getLocales('Сумма покупок'), dataIndex: 'purchasesSum', sort: true, key: 'operations', render: (e, item) => <span>{item.purchasesSum} {this.state.data.currency}</span>
            },
            {
                title: getLocales('Действие'), dataIndex: 'name', itemClassName: 'text-center', headerClassName: 'text-center', key: 'operations', render: (e, item) => <NavLink to={"/dashboard/shops/" + this.props.match.params.shopId + "/datas/users/" + item.id}><button className='btn btn-secondary font-m'>{getLocales('Перейти в профиль')}</button></NavLink>
            }
        ]
        return (
                ...
                <div class="block-body">
                    <div className="row">
                        <div className="col-lg-12">
                            <h3 className="font-m">{getLocales('Покупатели')}</h3>
                            {
                                            ...
                                            <div className="row">
                                                <div className="col-lg-6">
                                                    <div class="form-group">
                                                        <label class="form-control-label font-m">{getLocales('Платформа')}</label>
                                                        <select disabled={this.state.loading} value={this.state.sortPlatform} onChange={this.sort} name="sortPlatform" class="form-control">
                                                            ...
                                                            <option value="telegram">Telegram {getLocales('Бот')}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
